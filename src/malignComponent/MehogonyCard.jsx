...
export default function MehogonyCard() {
        ...
        {items.map((item) => (
          <Card className="my-3 malign-custom-card p-3 shadow-sm" key={item._id}>
            <Row className="no-gutters">
              ...
              <Col md={4} xs={12}>
                <Card.Img
                  src={`http://localhost:3000/malign-tree/mahogony/${item.image.split('/').pop()}`}
                  alt="Card image"
              ...
              <Col md={8} xs={12}>
                <Card.Body className="custom-malign-body-content">
                  ...
                  <Button as={Link} to={`/mehogony/${item._id}`} variant="primary">Read More</Button>
                </Card.Body>
              </Col>
            </Row>
          </Card>
