...
const LimitedActivitiseCard = () => {
  ...
  const fetchItems = async () => {
    const response = await axios.get("http://localhost:3000/api/limited-activitise");
  ...
  return (
    <div className="mx-1">
      {items.map((item) => (
        <Card className="my-3 p-3 shadow-sm" key={item._id}>
...
export default LimitedActivitiseCard;
